...
export default function KitchenDetails(props){
  ...
  useEffect(() => {
    ...
    // Fetch printer options from the backend
    const fetchPrinters = async () => {
      try {
        const response = await fetch(`${props.url}/pos/getPrinters`);
        if (response.ok) {
          const data = await response.json();
          setPrinterOptions(data.printers); // Update the printer options
        } else {
    ...
    fetchPrinters();
  }, [props.kitchenDetailsCopy, props.kitchenDetails, props.url, language]);
