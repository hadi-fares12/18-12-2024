...
def print_html(content, printer_name):
    ...
    with open("temp_print.html", "w", encoding="utf-8") as file:
        file.write(content)
    ...
    with open("temp_print.html", "rb") as file:
...
class PrintRequest(BaseModel):
    content: str  # HTML content to print
translations = {
    "en": {
        ...
        "Printer_updated": " updated successfully",
    ...
    "ar": {
        ...
        "Printer_updated": "تم تحديث  بنجاح",
    }
}
...
async def add_user(
    ...
    try:
        ...
        data = await request.json()
        insert_query = f"INSERT INTO users(username, password, user_control, email, sales, sales_return, purshase, purshase_return, orders, trans, items, chart, statement, SAType, Branch, COH, EOD, RecallInv,TableUser,LockAfterDone,Pos,ManageItems,ManageGrpItems,InvHistory,DailySales,CashOnHands,Description,UnitPrice,EndshiftEod,EndShiftCOH,AddandupdateSection,AddandUpdateTable,BarCodeUpdate,EditItemfrompos) VALUES (%s,%s ,%s,%s, %s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s)"
        cursor.execute(insert_query, (user_name_uppercase, '', '', '', "N", "N", "N", "N", "N", "N", "N", "N", "N", "SA", '', user_name_uppercase, "N", "N","N", "N","N", "N","N", "N","N", "N","UPrice","1","Y", "Y","Y", "Y","Y","N"))
...
# @app.post("/pos/printinv")
# async def print_receipt(request: PrintRequest):
#     try:
...
#             raise HTTPException(status_code=400, detail=f"Printer '{default_printer}' not found.")

#         # Prepare printable content
#         printable_content = request.content
...
#         bold_font = win32ui.CreateFont({"name": "Arial", "height": 30, "weight": 700})
#         small_font = win32ui.CreateFont({"name": "Arial", "height": 20, "weight": 500})

#         # Initial coordinates
#         x = 40  # left margin
#         y = 20  # top margin
#         max_y = 800  # Maximum Y position before a new page is needed
...
#         company_info = soup.find("div", {"style": "text-align: center; margin-bottom: 15px;"})
#         if company_info:
#             company_name = company_info.find("h2").get_text(strip=True)
#             dc.SelectObject(bold_font)
#             dc.TextOut(x, y, company_name)
#             y += 40  # Add margin for next line
#             check_page_break()

#             order_id = company_info.find("h4").get_text(strip=True)
#             dc.SelectObject(normal_font)
#             dc.TextOut(x, y, order_id)
#             y += 40
#             check_page_break()

#             # Print address and phone
#             address_phone = company_info.find_all("div")
#             for line in address_phone:
...
#         # Process Item List
#         item_rows = soup.find_all("div", {"style": "display: flex; flex-direction: column; border-bottom: 1px dotted black;"})
#         for row in item_rows:
#             details_row = row.find("div", {"style": "display: flex; justify-content: space-between;"})
#             if details_row:
#                 # Print the item details (Qty, U.P, Disc, Total)
#                 details = details_row.find_all("div")
#                 qty = details[0].get_text(strip=True)
#                 up = details[1].get_text(strip=True)
#                 disc = details[2].get_text(strip=True)
#                 total = details[3].get_text(strip=True)
#                 dc.SelectObject(normal_font)
#                 dc.TextOut(x, y, f"Qty: {qty}  U.P: {up}  Disc: {disc}%  Total: {total}")
#                 y += 40
#                 check_page_break()

#             # Print Item Name
#             item_name_div = row.find("div", {"style": "font-weight: bold; text-align: left; padding-top: 4px;"})
#             if item_name_div:
#                 item_name = item_name_div.get_text(strip=True)
#                 dc.SelectObject(bold_font)
#                 dc.TextOut(x, y, f"Item: {item_name}")
#                 y += 40
#                 check_page_break()

#         # Payment Summary Section
#         payment_summary = soup.find("div", {"style": "margin-top: 20px; border-top: 1px solid black; padding-top: 10px;"})
#         if payment_summary:
#             payment_summary_title = payment_summary.find("h4").get_text(strip=True)
#             dc.SelectObject(bold_font)
#             dc.TextOut(x, y, payment_summary_title)
#             y += 40
#             check_page_break()

#             # Summary of totals
#             payment_details = payment_summary.find_all("div", {"style": "display: flex; justify-content: space-between;"})
#             for detail in payment_details:
#                 if detail:
#                     label = detail.find("span").get_text(strip=True)
#                     value = detail.find_all("span")[1].get_text(strip=True)
#                     dc.SelectObject(normal_font)
#                     dc.TextOut(x, y, f"{label}: {value}")
#                     y += 40
#                     check_page_break()

#         else:
#             # Debugging log to check if payment summary is found
#             print("Payment summary not found.")

#         # Thank you message
#         thank_you_message = soup.find("h5")
#         if thank_you_message:
#             message = thank_you_message.get_text(strip=True)
#             dc.SelectObject(normal_font)
#             dc.TextOut(x, y, message)
#             y += 40
#             check_page_break()
...
#         text_content = soup.get_text("\n", strip=True)  # Extracts printable text while maintaining structure
...
#         # Define fonts
#         normal_font = win32ui.CreateFont({"name": "Arial", "height": 20, "weight": 400})
#         bold_font = win32ui.CreateFont({"name": "Arial", "height": 24, "weight": 700})
#         dc.SelectObject(normal_font)

#         # Print content
#         x, y = 40, 40  # Initial margins
#         line_spacing = 30
#         max_y = 1000  # Maximum Y position before page break
...
async def print_receipt(request: PrintRequest):
    try:
        ...
        sumatrapdf_path = r'C:\Users\USER\AppData\Local\SumatraPDF\SumatraPDF.exe'  # Path to SumatraPDF

        # Ensure wkhtmltopdf is available
        if not os.path.exists(wkhtmltopdf_path):
            raise HTTPException(status_code=500, detail="wkhtmltopdf.exe not found at specified path.")
        
        # Ensure SumatraPDF is available
        if not os.path.exists(sumatrapdf_path):
            raise HTTPException(status_code=500, detail="SumatraPDF.exe not found at specified path.")
        ...
        # Convert HTML content to PDF using wkhtmltopdf with better settings
        ...
        # Use SumatraPDF to print the generated PDF directly
        subprocess.run([sumatrapdf_path, '-print-to', default_printer, pdf_file.name], check=True)
...
def print_route():
    ...
    try:
        print_html(content, printer_name)
...
@app.post("/pos/updateinvoiceprinters/{company_name}")
async def update_printers(company_name: str, request: Request):
    language = request.headers.get("Language", "en")
    try:
        conn = get_db(company_name)
        cursor = conn.cursor()
        ...
        for item in data:
            if "PrinterName" not in item or "ReportName" not in item:
                raise HTTPException(status_code=400, detail="Invalid data format: PrinterName and ReportName are required.")

            # Update all rows without a WHERE clause
            query = """
                UPDATE invoiceprinters
                SET PrinterName = %s, ReportName = %s
            """
            cursor.execute(query, (item["PrinterName"], item["ReportName"]))

        conn.commit()
        return JSONResponse(
            content={"message": translations.get(language, {}).get("Printer_updated", "Printer updated successfully.")},
...
@app.get("/pos/getInvoicePrinterSettings/{company_name}")
async def get_invoice_printer_settings(company_name: str):
    try:
        conn = get_db(company_name)
        cursor = conn.cursor()
        
        # Query to fetch the current printer and report name settings
        query = "SELECT PrinterName, ReportName FROM invoiceprinters LIMIT 1"
        cursor.execute(query)
        result = cursor.fetchone()

        if result:
            printer_name, report_name = result
            return {"printerName": printer_name, "reportName": report_name}
        else:
            raise HTTPException(status_code=404, detail="No printer settings found.")
...
async def getLockAfterDoneRecallInv(company_name: str, username: str):
    try:
        ...
        pass
@app.get("/pos/getEditItemfrompos/{company_name}/{username}")
async def getEditItemfrompos(company_name: str, username: str):
    try:
        conn = get_db(company_name)
        cursor = conn.cursor()
        cursor.execute(f"SELECT EditItemfrompos FROM users WHERE username='{username}' ")
        COHR = cursor.fetchone()
        return {
            "EditItemfrompos": COHR[0],
        }
    except HTTPException as e:
        print("Error details:", e.detail)
        raise e
